...
# 由于每一个内容项div都包括在外层的page div里，此函数可在到达页面底部时跳到下一个页面，并且自动略过页眉页脚div
def find_next_div(current_div):
...
def make_the_table(first_td_div):
    ...
    if first_td_div:
        ...
        final_original_table,now_current_div,paging_tag = build_original_table(first_td_div)

        table2str_temp1 = table2str_temp1P.sub('', final_original_table)
...
# 传入当前的current——div，返回值为最终的str,当前的位置 now_current_div 还有是否翻页的标志paging_tag
def find_the_table(currnet_div):
    ...
    return make_the_table(first_td_div),now_current_div,paging_tag

# 通过传入 入口 参数， 找出所有的表格，直到出口，输出 original——table,还有最终出口的 chukou_div 的位置,还有是否翻页的信息paging_tag
def build_original_table(current_td_div):
    ...
    # print(chukou_div)
    chukou_div = current_td_div
    return original_table,chukou_div,paging_tag
...
def find_the_current_div_else_inEX(html_filename):
    ...
    for b in range(int(len(pf) * 0.6), int(len(pf) * 0.9), 1):
        ...
        temp_div = page_temp[0]
        # print(temp_div)
        # for div_num in (len(page_temp)-1):
        #     temp_div = page_temp[div_num]
        ...
        while temp_div:
            if len(re.findall(r'.*应付职工薪酬列示|应付职工薪酬|应付职工薪酬情况|应付职工薪酬|应付职工薪酬分类\b',temp_div.get_text())) > 0 and has_all_table == 0:
                ...
                has_all_table = 1
                # print(current_div)
                has_all_table_str, now_current_div, paging_tag = find_the_table(current_div)
                temp_div = now_current_div
                start_count = 1
            elif len(re.findall(r"短期薪酬情况|短期薪酬列示[：|:]?|短期薪酬",temp_div.get_text())) > 0 and has_all_table and has_short_pay_table == 0:
                current_div = temp_div
                has_short_pay_table = 1
                has_short_pay_str ,now_current_div,paging_tag= find_the_table(current_div)
                temp_div = now_current_div
                # start_count = 1
            elif len(re.findall(r"提存计划情况|提存计划列示[：|:]?|提存计划",temp_div.get_text())) > 0 and has_all_table and has_plan_pay_table == 0:
                current_div = temp_div
                has_plan_pay_table = 1
                has_plan_pay_str ,now_current_div,paging_tag= find_the_table(current_div)
                temp_div = now_current_div
                return has_all_table_str, has_short_pay_str, has_plan_pay_str

            # elif has_all_table and paging_tag == 1:
            else:
    ...
    if index_div:
        ...
        if staff_pay_a:
            ...
            if staff_pay_page_number:
                ...
                temp_div = staff_pay_page_content[0]
                if paging_tag == 1:
                    temp_div = now_current_div
                while temp_div:
                    if len(re.findall(r'.*应付职工薪酬列示|应付职工薪酬|应付职工薪酬情况|应付职工薪酬|应付职工薪酬分类\b',
                        ...
                        temp_div = now_current_div
                        start_count = 1
                    elif len(re.findall(r"短期薪酬情况|短期薪酬列示[：|:]?|短期薪酬",
                                        temp_div.get_text())) > 0 and has_all_table and has_short_pay_table == 0:
                        current_div = temp_div
                        has_short_pay_table = 1
                        has_short_pay_str, now_current_div, paging_tag = find_the_table(current_div)
                        temp_div = now_current_div
                    elif len(re.findall(r"提存计划情况|提存计划列示[：|:]?|提存计划",
                                        temp_div.get_text())) > 0 and has_all_table and has_plan_pay_table == 0:
                        current_div = temp_div
                        has_plan_pay_table = 1
                        has_plan_pay_str, now_current_div, paging_tag = find_the_table(current_div)
                        temp_div = now_current_div
