...
def find_the_current_div_else_inEX(html_filename):
    ...
    pf = soup.findAll(class_='pf')
    print("进入了没有目录的模式")
    for i in range(int(len(pf) * 0.6),int(len(pf) * 0.9),  1):
        ...
        for temp_div in page_temp:
            has_all_pay_title_len = len(re.findall(r'应付职工薪酬列示|应付职工薪酬|应付职工薪酬情况|应付职工薪酬\b', temp_div.get_text()))
            has_short_pay_title_len = len(re.findall(r"短期薪酬列示|短期薪酬情况\b", temp_div.get_text()))
            has_plan_pay_title_len = len(re.findall(r'提存计划列示|提存计划情况', temp_div.get_text()))
            if len(re.findall(r'.*应付职工薪酬列示|应付职工薪酬|应付职工薪酬情况|应付职工薪酬\b', temp_div.get_text())) > 0 and has_all_table == 0:
                ...
                continue
            if len(re.findall(r"短期薪酬列示|短期薪酬情况\b", temp_div.get_text())) > 0 and has_short_pay_table == 0:
                current_div = temp_div
                # print(current_div)
                # print('找到短期薪酬列示的标题')
                ...
                continue
            if len(re.findall(r'提存计划列示|提存计划情况\b', temp_div.get_text())) > 0 and has_plan_pay_table == 0:
        ...
        if not (has_all_table and has_short_pay_table and has_plan_pay_table):
            page_temp_pc = pf[i+1].find('div')
            staff_pay_page_next1_content = page_temp_pc.find_all('div')
            for temp_div in staff_pay_page_next1_content:
                if has_all_table == 0:
                    # print("没有职工薪酬的表格")
                    if len(re.findall(r'.*应付职工薪酬列示|应付职工薪酬|应付职工薪酬情况|应付职工薪酬\b', temp_div.get_text())) > 0:
                        current_div = temp_div
                        has_all_table = 1
                        # print(current_div)
                        # print('找到应付职工薪酬列示的标题')
                ...
                if has_short_pay_table == 0:
                    if len(re.findall(r"短期薪酬列示|短期薪酬情况\b", temp_div.get_text())) > 0:
                        current_div = temp_div
                        # print(current_div)
                        # print('找到短期薪酬列示的标题')
                        has_short_pay_table = 1
                ...
                if has_plan_pay_table == 0:
                    if len(re.findall(r'提存计划列示|提存计划情况\b', temp_div.get_text())) > 0:
                        has_plan_pay_table = 1
                        # print('找到提存计划列示的标题')
                        # print(current_div)
                        current_div = temp_div
        ...
        if staff_pay_a:
            ...
            if staff_pay_page_number:
                ...
                for temp_div in staff_pay_page_content:
                    has_all_pay_title_len = len(re.findall(r'.*应付职工薪酬列示|应付职工薪酬|应付职工薪酬情况|应付职工薪酬\b', temp_div.get_text()))
                    has_short_pay_title_len = len(re.findall(r"短期薪酬列示|短期薪酬情况\b", temp_div.get_text()))
                    has_plan_pay_title_len = len(re.findall(r'提存计划列示|提存计划情况\b', temp_div.get_text()))
                    if len(re.findall(r'.*应付职工薪酬列示|应付职工薪酬|应付职工薪酬情况|应付职工薪酬\b', temp_div.get_text())) > 0 and has_all_table == 0:
                        current_div = temp_div
                        has_all_table = 1
                        # print(current_div)
...
fiscal_period_dict = {"html/2016/YEAR/":"2016-12-31","html/2015/YEAR/":"2015-12-31","html/2007/YEAR/":"2007-12-31", \
                      "html/2008/YEAR/":"2008-12-31","html/2009/YEAR/":"2009-12-31","html/2010/YEAR/":"2010-12-31", \
                      "html/2011/YEAR/": "2011-12-31","html/2012/YEAR/":"2012-12-31","html/2013/YEAR/":"2013-12-31", \
                      "html/2014/YEAR/": "2014-12-31"}
...
for i in range(len(path)):
     ...
     if os.path.exists(path[i] + 'payBaseInformation.txt'):
         os.remove(path[i] + 'payBaseInformation.txt')
     fileflow = open(path[i] + 'payBaseInformation.txt','a+',encoding='utf-8')
     for filename in files:
         pos = filename.find(".")
         if filename[pos + 1:] == "html":
             print(path[i] + filename + "正在提取...")
             short_name,fiscal_period,share_code,company = base_information_extract(path[i] + filename)
             ...
             has_all_table_str, has_short_pay_str, has_plan_pay_str = staff_pay_information_extract(path[i] + filename)
